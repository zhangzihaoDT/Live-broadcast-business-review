...
该报告展示了直播业务的核心经济指标、规模敏感性分析、场景差异分析、投放边际收益分析及每日表现分析。

- **生成脚本**: `/Users/zihao_/Documents/github/直播运营复盘/Dashboard_Step3_Report.py`
- **主要功能**:
  - 加载场次线索数据，计算核心指标（总场次、总消耗、CPL等）。
  - 执行四个维度的分析：
    1. **规模敏感性分析 (Scale Sensitivity)**: 分析不同规模直播场次的经济效益
    2. **场景差异分析 (Scenario Difference)**: 对比大促vs日常直播的表现差异
    3. **投放边际收益分析 (Marginal Return)**: 评估广告投放的边际效益
    4. **每日表现分析 (Daily Performance)**: 分析指定时间区间内每日的线索数、CPL和直播场数
  - 调用 `Dashboard_Step2_Conversion.py` 获取转化归因数据。
  - 生成包含 Plotly 图表和分析结论的综合仪表盘。

### 每日表现分析模块特性

该模块支持通过 `--start-date` 和 `--end-date` 参数分析指定时间范围内的每日表现：

1. **时间区间支持**：模块支持 `--start-date` 和 `--end-date` 参数，可以分析指定时间范围内的每日表现
2. **三个核心指标**：
   - 每日线索数 (Daily Leads)
   - 每日CPL (Cost Per Lead)
   - 每日直播场数 (Daily Sessions)
3. **交互式可视化**：使用Plotly生成包含三个子图的交互式图表，支持缩放、悬停查看详细数据
4. **详细洞察分析**：提供每日统计数据表格、总体表现总结、最佳/最差表现日分析、趋势分析和优化建议
5. **HTML报告集成**：新模块已完全集成到现有的HTML报告中，作为第4个分析部分出现

### 技术实现细节

- **数据分组**：使用 `df.groupby('date')` 按日期分组计算每日统计数据
- **CPL计算**：`CPL = 广告消耗 / unique_leads`
- **图表布局**：使用 `make_subplots()` 创建3行1列的垂直布局
- **时间过滤**：支持通过 `start_date` 和 `end_date` 参数过滤数据
- **错误处理**：包含对空数据和除零错误的处理
...
- **主要功能**:
  ...
  - **同类群组分析 (Cohort Analysis)**: 筛选指定时间范围内的"IM智己汽车"渠道用户。
  - **归因逻辑**:
...
- **使用方法**:
  ...
  # 运行管道，指定渠道和时间范围
  python Dashboard_Pipeline_Runner.py \
    --excel source/IM智己汽车直播复盘表2026-02-27.xlsx \
    --csv source/IM智己汽车\ 0201～0227.csv \
    --channel "IM智己汽车" \
...
- **参数说明**:
  ...
  - `--channel` / `-c`: （可选）目标渠道名称。如果不指定，系统会根据文件名自动检测：
    - 文件名包含 "IM智己汽车" → 渠道为 "IM智己汽车"
    - 文件名包含 "快慢闪" 或 "CM2" → 渠道为 "Popup（门店）"
    - 其他情况 → 使用 CSV 文件中最常见的渠道
  - `--start-date` / `-s`: （可选）分析开始日期，格式为 YYYY-MM-DD
  - `--end-date` / `-e`: （可选）分析结束日期，格式为 YYYY-MM-DD

- **输出结果**:
  运行成功后，将自动生成以下文件：
  1. **Dashboard Report**: `dashboard_report.html` (位于项目根目录)，包含四个分析维度：
     - 规模敏感性分析
     - 场景差异分析
     - 投放边际收益分析
     - 每日表现分析（如果指定了时间范围）
